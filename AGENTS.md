...
- DOM primitives reside in `src/dom/mod.rs`: `Node::{Element, Text}`, `ElementKind::{Column, Row, Block}`, `Style` and `Attributes`. Helpers include `text`, `column`, `row`, `block`, with builder-like APIs (`Node::with_style`, `Attributes::with_style`, etc.). Layout is minimal—columns stack children, rows share width evenly, blocks add a border and render inner content.
