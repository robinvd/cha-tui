...
use chatui::components::input::HighlightSpan;
#[cfg(test)]
use chatui::dom::TextSpan;
use chatui::dom::{Color, Style};

use color_eyre::eyre::{Result, WrapErr, eyre};
use tree_house::tree_sitter::InputEdit;
...
pub struct DocumentHighlighter {
    ...
    language: TreeHouseLanguage,
    syntax: Syntax,
}

impl DocumentHighlighter {
    pub fn new(path: &Path, rope: &Rope) -> Result<Option<Self>> {
        let Some(language_name) = language_for_path(path) else {
            return Ok(None);
        };

        let loader = TreeHouseLoader::new()?;
        let language = loader
            .language_for_name(language_name)
            .wrap_err_with(|| format!("failed to resolve language '{language_name}'"))?;
        let syntax = create_syntax(&loader, language, rope, language_name)?;
    ...
    pub fn rebuild(&mut self, rope: &Rope) -> Result<Vec<HighlightSpan>> {
        self.syntax = create_syntax(&self.loader, self.language, rope, self.language_name)?;
        self.highlight_all(rope)
    }

    pub fn apply_edits(&mut self, rope: &Rope, edits: &[InputEdit]) -> Result<Vec<HighlightSpan>> {
        if edits.is_empty() {
            return self.highlight_all(rope);
        }
        ...
        self.syntax
            .update(
                slice,
                Duration::from_millis(SYNTAX_TIMEOUT_MILLIS),
                edits,
                &self.loader,
            )
            .map_err(|err| {
                eyre!(
                    "failed to update syntax for '{}': {err}",
                    self.language_name
                )
            })?;
    ...
    pub fn highlight_all(&self, rope: &Rope) -> Result<Vec<HighlightSpan>> {
        collect_highlight_spans(&self.loader, &self.syntax, rope)
    }
}
...
fn collect_highlight_spans(
    ...
    let rope_copy = Rope::from_str(&text);
    let slice = rope_copy.slice(..);
    let palette = loader.palette();
    let byte_len = u32::try_from(text.len()).unwrap_or(u32::MAX);
    let mut highlighter = TreeHouseHighlighter::new(syntax, slice, loader, 0..byte_len);
    ...
    loop {
        ...
        let next = usize::min(next_offset as usize, total_bytes);
        if next > byte_pos {
            if let Some(fragment) = text.get(byte_pos..next) {
                let fragment_chars = fragment.chars().count();
                if fragment_chars > 0 {
                    let style = style_stack.last().cloned().unwrap_or_else(Style::default);
                    push_highlight_span(&mut spans, char_pos, char_pos + fragment_chars, style);
                    char_pos += fragment_chars;
                }
            }
            byte_pos = next;
        }
    ...
    if byte_pos < total_bytes
        && let Some(fragment) = text.get(byte_pos..)
    {
        let fragment_chars = fragment.chars().count();
        if fragment_chars > 0 {
            let style = style_stack.last().cloned().unwrap_or_else(Style::default);
...
fn push_highlight_span(spans: &mut Vec<HighlightSpan>, start: usize, end: usize, style: Style) {
    if start >= end {
        return;
    }
